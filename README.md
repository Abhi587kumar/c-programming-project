...
if (file == NULL) {
printf("[SYSTEM] Data file '%s' not found. Starting with an empty
list.\n", DATA_FILE);
return;
}
...
if (studentCount > 0) {
size_t records_read = fread(students, sizeof(struct Student),
studentCount, file);
...
if (scanf("%d", &s->marksC) != 1 || s->marksC < 0 || s->marksC > 100)
s->marksC = 0;
printf("Enter Marks for Mathematics (0-100): ");
if (scanf("%d", &s->marksMaths) != 1 || s->marksMaths < 0 || s-
>marksMaths > 100) s->marksMaths = 0;
printf("Enter Marks for Physics (0-100): ");
if (scanf("%d", &s->marksPhysics) != 1 || s->marksPhysics < 0 || s-
>marksPhysics > 100) s->marksPhysics = 0;
printf("Enter Marks for Electronics (0-100): ");
if (scanf("%d", &s->marksElectronics) != 1 || s->marksElectronics < 0
...
void assignGrade(float percentage, char *gradeBuffer) {if (percentage >= 90.0f) {
